 ...
 */
  ...
  function wordsList(str, subStr) {
    let arr = str.replace(/[.,!?;:"]/gi, '').toLowerCase().split(' ');
    let newArr = [];
    for (let str of arr) {
    ...
    return newArr.filter(item => item.includes(subStr));
  }
...
function getLocalDate(date, isSeconds, isISO) {
    if (!isSeconds && !isISO) {
        return date.toLocaleString('ru-RU').substr(0, 17);
    }
    if (isSeconds && !isISO) {
        return date.toLocaleString('ru-RU').substr(0, 20);
    }
...
//   return date.toLocaleString('ru-RU').substr(0, 10);
// }
...
function getDateAgo(date, days) {
  ...
  return myDate.toLocaleString('ru-RU').substr(0, 10);
}
...
let Car = function (engine, model, name, year) {
  ...
  let car = new Car(1600, 'Accent', 'Hyndai', 2019);
  let car2 = new Car(5000, 'Huracán', 'Lamborghini', 2020);
...
let testPerformance = (iterations, func) => {
  var time = Date.now();
  if (typeof func === 'function') for (let i = iterations; ;i--) {
    func();
  }
